LArray
=== 
A library for managing large off-heap arrays that can hold more than 2G (2^31) entries in Java and Scala. Notablly LArray is *disposable* by calling `LArray.free` or let GC automatically release the memory. LArray also has `mmap` (memory-mapped file) support for more than 2GB of files. 
